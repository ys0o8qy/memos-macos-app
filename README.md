...
- 可选全局快捷键 `Control Option M`，保存 `Command Enter`，编辑保存 `Command S`，收起 `Esc`。
...
第一版没有后台离线上传、自动更新、开机启动、多账号同时登录。全局快捷键可以开关，组合固定。预览支持标题、列表、任务项和行内 Markdown；复杂表格、代码块及正文内的远程图片尚未完整渲染，原始 Markdown 会原样保存。

## 测试与手工验证

17 项自动测试覆盖：认证错误、URL/CEL 编码、账号过滤与分页、默认私有、编辑字段范围、图片 Base64 上传、重复请求恢复、附件认证边界、草稿恢复与隔离、坏文件保护、离线恢复、编辑冲突、独立剪贴板图片解码、原生界面离屏渲染。
